...
beforeEach(function () {
    ...
    // Every attendance migration except the permission grants (no role_permissions
    // here). The 09-11 batch matters as much as the 09-10 one: it adds the periods
    // and exports tables plus `locked` on attendance_days, and
    // AttendanceDayProcessor reads attendance_periods through PeriodLocks on every
    // build — so without them the processor fails with "no such table".
    foreach (array_merge(
