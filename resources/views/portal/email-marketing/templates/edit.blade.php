...
<div class="em-editor-fullbleed">
    <div class="px-3 px-lg-4 pt-4">
        ...
        <form id="template-form"
            ...
            {{-- ── SAMIR icon library (managed via Portal → Email Marketing → Icon library) ───
                 Click any icon to copy a snippet; paste into an Unlayer HTML block. --}}
            <div class="card shadow-sm mb-3 border-warning">
                <div class="card-body py-2">
                    <div class="d-flex align-items-center flex-wrap gap-2">
                        <strong class="text-warning"><i class="bi bi-stars me-1"></i>SAMIR icon library</strong>
                        <small class="text-muted me-2">Click to copy a snippet.</small>
                        @forelse ($icons ?? [] as $icon)
                            <button type="button" class="btn btn-sm btn-outline-secondary copy-icon"
                                    data-snippet="{{ $icon->toHtmlSnippet() }}" title="Click to copy {{ $icon->label }}">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"
                                     stroke="{{ $icon->default_color }}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="{{ $icon->svg_path }}"/>
                                </svg>
                                <small class="ms-1">{{ $icon->label }}</small>
                            </button>
                        @empty
                            <small class="text-muted">No icons yet —
                                <a href="{{ route('portal.marketing.icons.create') }}">add one</a>.
                            </small>
                        @endforelse
                        <a href="{{ route('portal.marketing.icons.index') }}" class="btn btn-sm btn-outline-warning ms-1" title="Manage icon library">
                    ...
                    <small class="text-muted d-block mt-1">
                        After pasting, edit <code>stroke="#…"</code> for color and <code>width/height</code> for size.
                        Manage the library under <a href="{{ route('portal.marketing.icons.index') }}">Icon library</a>.
                    </small>
                </div>
            </div>
...
(function () {
    ...
    function initUnlayer() {
        ...
        // Custom fonts managed via Portal → Email Marketing → Fonts. Unlayer's
        // `customFonts` API takes {label, value, url} objects — value is the
        // CSS font-family, url is a stylesheet that loads it.
        const customFonts = @json(($fonts ?? collect())->map->toUnlayerCustomFont()->values());
