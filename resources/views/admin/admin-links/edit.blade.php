...
<div class="d-flex align-items-center mb-4">
    <div>
        <h1 class="h3 mb-0 fw-bold">Edit Admin Link</h1>
        <small class="text-muted">Update {{ $adminLink->name }}</small>
    </div>
    <div class="ms-auto">
...
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form method="POST" action="{{ route('admin.admin-links.update', $adminLink) }}">
            @csrf
            @method('PUT')
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                           value="{{ old('name', $adminLink->name) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <select name="category_id" class="form-select @error('category_id') is-invalid @enderror" required>
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('category_id', $adminLink->category_id) == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="sort_order" class="form-control"
                           value="{{ old('sort_order', $adminLink->sort_order) }}" min="0">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Status</label>
                    <div class="form-check form-switch mt-2">
                        <input class="form-check-input" type="checkbox" name="is_active" value="1"
                               {{ old('is_active', $adminLink->is_active) ? 'checked' : '' }} id="isActive">
