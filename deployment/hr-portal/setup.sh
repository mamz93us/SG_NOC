#!/usr/bin/env bash
# =============================================================================
# Provision the NOC VM for the HR portal subdomain.
#
...
# `git pull`.
#
#   sudo bash deployment/hr-portal/setup.sh
...
#   sudo SKIP_TLS=1 bash deployment/hr-portal/setup.sh        # HTTP only, cert later
#
...
#
# See ../../docs/HR_PORTAL.md for the full runbook, including the Entra redirect
# URI that must be registered separately.
# =============================================================================
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_DIR="${APP_DIR:-$(cd "$HERE/../.." && pwd)}"
DOMAIN="${HR_PORTAL_DOMAIN:-hr.samirgroup.net}"
...
cat > "$AVAILABLE" <<NGINX
# HR portal subdomain — managed by deployment/hr-portal/setup.sh
...
if [[ "$DOMAIN" != "hr.samirgroup.net" ]]; then
    log "Pinning HR_PORTAL_DOMAIN=$DOMAIN in .env"
    if grep -q '^HR_PORTAL_DOMAIN=' "$APP_DIR/.env" 2>/dev/null; then
        sed -i "s|^HR_PORTAL_DOMAIN=.*|HR_PORTAL_DOMAIN=${DOMAIN}|" "$APP_DIR/.env"
    else
        printf '\nHR_PORTAL_DOMAIN=%s\n' "$DOMAIN" >> "$APP_DIR/.env"
    fi
fi
...
# The HR permission slugs must exist before anyone can reach the workspace.
log "Running migrations (as $APP_OWNER) ..."
sudo -u "$APP_OWNER" php "$APP_DIR/artisan" migrate --force

# --- Smoke test ---------------------------------------------------------------
# Hit the local nginx with the HR Host header. This proves both halves at once:
...
cat <<REMINDER
  ...
  Then grant HR staff access: Admin -> Users -> set role to "hr" (or tick
  manage-hr-portal / submit-hr-* in Admin -> Permissions).
