...
class EmployeeCardController extends Controller
{
    ...
    public function show(string $token): Response|\Illuminate\Contracts\View\View
    {
        ...
        $data = [
            ...
            // Canonical (card host) for anything shared outward; the download links
            // stay on the current host so they work wherever the page is being viewed.
            'card_url' => VCard::cardUrl($token),
    ...
    private function makeQr(string $url): string
    {
        ...
        // outputBase64 defaults to true (returns a data: URI) — disable it so render()
        // returns raw inline <svg> markup we can drop straight into the DOM.
