...
class PrinterScriptService
{
    ...
    public function generateWindowsBat(Printer $printer, ?PrinterDriver $driver): string
    {
        ...
        return "@echo off\r\n"
            ...
            . "echo ============================================\r\n"
            . "echo.\r\n"
            . "echo [1/2] Creating TCP/IP printer port...\r\n"
            . "cscript /nologo \"%SystemRoot%\\System32\\Printing_Admin_Scripts\\en-US\\prnport.vbs\" -a -r \"{$portName}\" -h \"{$ip}\" -o raw -n 9100\r\n"
            . "IF !ERRORLEVEL! NEQ 0 (\r\n"
            . "    echo WARNING: Port may already exist. Continuing...\r\n"
            . ")\r\n"
            . "echo [2/2] Adding printer...\r\n"
            . "rundll32 printui.dll,PrintUIEntry /in /n \"\\\\{$ip}\\{$shareName}\"\r\n"
            . "IF !ERRORLEVEL! NEQ 0 (\r\n"
            . "    echo Trying PowerShell method...\r\n"
            . "    powershell -Command \"Add-PrinterPort -Name '{$portName}' -PrinterHostAddress '{$ip}'; Add-Printer -Name '{$printerName}' -DriverName 'Microsoft IPP Class Driver' -PortName '{$portName}'\"\r\n"
            . ")\r\n"
            . "echo.\r\n"
            . "echo ============================================\r\n"
            . "IF !ERRORLEVEL! NEQ 0 (\r\n"
            . "    echo  Some steps failed. Contact IT if printer not working.\r\n"
            . ") ELSE (\r\n"
            . "    echo  DONE! {$printerName} is ready to use.\r\n"
            . ")\r\n"
            . "echo ============================================\r\n"
            . "pause\r\n";
    }
