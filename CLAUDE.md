...
- **Create Ticket** — raises a ticket in the external Samir ticketing system from `/admin/tickets/create`. `Services\Ticketing\NocTicketService` (multipart POST to `addTicketingRequestForNOC`), `TicketCatalog` (the hand-maintained ID→label map, since the API lists nothing), `Admin\NocTicketController`, `NocTicket` (local submit log — failures are rows too). Endpoint, key and catalog live in Settings, **not** `.env`. Distinct from the onboarding `TicketingApiService`, which calls `provisionNewEmployee` on a different path. Two traps: the API's own field is spelled **`ticketPriotity`** (do not "fix" it — the priority arrives unset), and the requester is identified by **Azure object id**, so they must exist in `identity_users`. See [CREATE_TICKET_SETUP.md](CREATE_TICKET_SETUP.md).
