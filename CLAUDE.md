...
- **Access points** — `AccessPoint` model + `Admin\AccessPointController`, page at `/admin/network/access-points`. APX-series APs have no SNMP, so status is ICMP only (`access-points:ping`, every 5 min). Rows arrive from a Sophos Central CSV **or** the manual Add form; both go through `AccessPointImporter::ensureAsset()`, which links or creates the ITAM `Device` and assigns an asset code via `AssetCodeService` — keep any new entry point on that one path so codes stay in one sequence.
...
- **Telephony** — UCM integration via `SyncUcmExtensionsJob`, `SyncUcmActiveCallsJob`, both called inline from the scheduler. Not SOAP: `Services\IppbxApiService` is JSON-over-HTTPS to `{url}/api` with an MD5 challenge/cookie handshake. There is **no** `Phone`, `Extension`, `Trunk` or `ActiveCall` model — phones are `Device` rows with `type='phone'`, and the rest are `UcmExtensionCache` / `UcmTrunkCache` / `UcmActiveCall`. `UcmServer` is real. GDMS cloud side: `Services\GdmsService` — see [GDMS_PHONE_MANAGEMENT.md](GDMS_PHONE_MANAGEMENT.md).
- **Phone firmware server** — the NOC hosts Grandstream firmware over plain HTTP so a path set once in the UCM Zero Config global policy upgrades the whole fleet. Bytes are served by **nginx**, never Laravel (`deployment/firmware/setup.sh` writes an internal port-80 vhost plus a `/fw/` alias in `sites-dynamic`); the app only manages the library (`PhoneFirmware`, `Services\Phone\FirmwarePublisher`, `/admin/phones/firmware`) and reports state. Phones self-report their running version in the `SW` field of the User-Agent they send to `/phonebook.xml`, which `PhonebookController` records — that, not GDMS, drives the status board. **Never put the firmware vhost behind an HTTP→HTTPS redirect**: old phone firmware fails modern TLS and does not follow 302s, so every phone silently stops upgrading. See [PHONE_FIRMWARE_SERVER.md](PHONE_FIRMWARE_SERVER.md).
- **CUPS printers** — `CupsRefreshStatus` command, `CupsPrinter` / `CupsPrintJob` models.
- **Create Ticket** — raises a ticket in the external Samir ticketing system from `/admin/tickets/create`. `Services\Ticketing\NocTicketService` (multipart POST to `addTicketingRequestForNOC`), `TicketCatalogApi` (pulls categories/sub-categories from the API's sibling `getCategoriesForNOC` / `getSubCategoriesForNOC` endpoints (the plain `getCategories` pair is JWT-guarded and rejects the API key), cached 6h), `TicketCatalog` (merges that with the JSON in Settings, which still supplies the **type/priority labels** the API only exposes as bare ids), `Admin\NocTicketController`, `NocTicket` (local submit log — failures are rows too). Endpoint, key and catalog live in Settings, **not** `.env`; the lookup base is derived from the submit URL by dropping its last path segment, so there is one URL field, not three. Distinct from the onboarding `TicketingApiService`, which calls `provisionNewEmployee` on a different path. Reading tickets back is `TicketRequestService` over `getTicketingRequestsByStatusForNOC` / `getTicketingRequestDetailsForNOC`, surfaced at **`/admin/tickets/tracker`** and on the home portal at **`/tickets`** (`Admin\TicketTrackerController`, `Home\HomeTicketController`) — distinct from `/admin/tickets`, which is only the local submit log. The list endpoint returns whole ticket objects, so per-status counts come from one `requestStatus=0` call grouped locally, not eight. `TicketingApiClient` is the shared base-URL/key/GET layer. Three traps: the priority field is spelled **`ticketPriotity`** when submitting and in the list but **`ticketPriotiry`** in the details — both are the API's own typos, do not "fix" either; the requester is identified by **Azure object id**, so they must exist in `identity_users`; and `getTicketingRequestDetailsForNOC` takes a bare `requestId` **without checking who is asking**, so both controllers verify the ticket is in that person's own list first — removing that turns either page into an enumeration of everyone's tickets. See [CREATE_TICKET_SETUP.md](CREATE_TICKET_SETUP.md).
- **Employee home portal** — `home.samirgroup.net`, the page every company PC opens on. Fourth isolated subdomain (same pattern as vcard/hr/em): `config/home_portal.php`, `Support\HomePortal`, `EnforceHomePortalHostIsolation`, `Home\HomeController`. **Signs people in as their Windows account with no click** — one redirect to Entra with `prompt=none`, which an Entra-joined machine satisfies from its Primary Refresh Token. Entra declining that is normal, not an error: the callback sets a **short-lived loop-guard cookie** (`HomePortal::SILENT_OFF_COOKIE`) and renders a sign-in page. **Never remove that cookie** — without it a browser that cannot sign in silently bounces against login.microsoftonline.com on every launch, on every PC. 2FA is skipped by HOST and the session is never marked `2fa_verified`. **Edge does PRT SSO natively; Chrome needs `CloudAPAuthEnabled=1`** or a third of the fleet silently falls back to a sign-in button. The page is three bands — **Quick access** (Oracle/Salesforce/ArcMate from `core_systems`, plus Outlook from `webmail_url`), **IT & support** and **Company**. Cards: announcements (slider + `Announcement`/`AnnouncementRead`), `CompanyEvent` (hourly `company-calendar:sync`, needs Calendars.Read **application** consent), `Knowbe4Score` (daily `knowbe4:sync`, region-specific API host — wrong region 401s like a bad token), payday countdown, My Assets, and the employee card — which offers **both Apple and Samsung Wallet** (`WalletPassService` builds a signed .pkpass file; `SamsungWalletService` builds a signed LINK, `https://a.swallet.link/atw/v3/{cardId}#Clip?cdata={jws}` — the `#Clip` fragment before the query is Samsung's, not a typo). Both buttons open a QR pointing at a short **signed 15-minute** route on our own host, never the pass or the 1.5 KB Samsung link itself. Samsung needs an approved partner account and a published **Generic** template whose field LABELS live in the Portal while the NOC sends only values — see [EMPLOYEE_CARD_SETUP.md](EMPLOYEE_CARD_SETUP.md). **Documentation & Manuals / IT Policy** are one library — `PortalDocument`, `Home\HomeDocumentController`, authored at `/admin/portal-documents` (`manage-portal-documents`) — split only by `category`. A row is a file, a **YouTube video** or a link; PDFs, images and videos open in an in-app viewer (`/documents/{id}`, iframe fed by `/documents/{id}/file` with `Content-Disposition: inline`), everything else downloads. Uploads live on the **`private` disk** and are served solely by those three routes, each re-checking publication and audience per request, so never move them to the public disk to "fix" a download. Only an id returned by `PortalDocument::youtubeId()` is ever put in an iframe src — never the pasted URL. Nothing calls an external API on request — the whole company loads this at 9am. See [HOME_PORTAL_SETUP.md](HOME_PORTAL_SETUP.md).
...
- **Branch tunnel watchdog** — `TunnelWatchdog` service + `tunnel-health:watch` command (every minute), `TunnelHealthController`, page at `/admin/network/tunnel-health`. Models: `BranchTunnel` (gateway firewall), `TunnelProbe` (one per carried subnet), `TunnelHealthCheck` (per-cycle history, pruned at 7 days), `TunnelOutage` (one row per down/degraded stretch, **never pruned**). Three views: the live board, the 7-day hourly grid (`TunnelHealthHistoryController`), and the **outage report** at `/admin/network/tunnel-health/report` (`TunnelOutageReportController`) — incident log with exact from/to times, per-tunnel availability, CSV export and print layout, for raising tickets and SLA credit claims with the ISP. The outage log is written on state transitions and is deliberately *not* derived from `NocEvent`: events start late (`ALERT_AFTER_FAILURES`) and the flap window merges separate incidents, both right for paging and wrong for billing. `tunnel-health:backfill-outages` reconstructs incidents from whatever check history exists. Replaced the strongSwan **VPN Hub**, which could start/stop tunnels the NOC no longer owns — see the gotcha below.
...
- **Every external port-80 request lands on the firmware vhost.** It binds `listen 172.16.8.11:80` — a SPECIFIC address — while every other vhost uses wildcard `listen 80`, and nginx picks a server block by listen specificity **before** server_name. Azure NATs inbound traffic to that private address, so Host is never consulted. Two consequences, both of which have already bitten: (1) `http://<any-site>/` returns the firmware vhost's 404 instead of certbot's HTTP→HTTPS redirect, because that redirect sits in a wildcard block nothing reaches — fix is a server block on the *same* specific address for that one server_name (`zz-home-http` is the worked example); (2) **certbot's `--nginx` authenticator cannot validate anything on this host**, so use `-a webroot -w /var/www/acme` (the firmware vhost serves that path via a `^~` location that beats its dotfile deny). The six certs predating this still have `authenticator = nginx` in `/etc/letsencrypt/renewal/*.conf` and **will fail to renew** until switched. Never add a redirect to the firmware vhost itself, and keep it first in `sites-enabled` so it stays the default_server — phones fetch by bare IP and cannot follow 302s.
- **`/phonebook.xml` ignores host isolation**, on purpose: it is registered `->withoutMiddleware(['web'])` so desk phones can fetch it without a session, and all four `Enforce*HostIsolation` middlewares live in the `web` group. It answers 200 on vcard, hr, em and home alike. That is the public directory phones already read unauthenticated, not a leak — but do not cite those middlewares as sealing a host completely.
- **Split-brain DNS: the NOC cannot resolve public `*.samirgroup.com` hosts.** `resolved.conf.d/sg-dns.conf` routes every query (`Domains=~.`) to the internal AD DNS `172.16.8.4/.5`, which is authoritative for `samirgroup.com` and holds only internal records. Anything published solely in the public zone — `sgprd`, `sgapps-test` — comes back `cURL error 6: Could not resolve host`, which reads like a dead host but is NXDOMAIN from an authoritative server. `dig @1.1.1.1` proves it resolves publicly. `deployment/dns/` fixes it with a **dnsmasq** forwarder on 127.0.0.1 (resolved's upstream) that sends only those names to a public resolver; add new ones as `server=/name/1.1.1.1` lines, never by pinning `/etc/hosts` (the WAF IPs are anycast and rotate between calls). The systemd-resolved routing-domain-on-a-dummy-link approach looks right and is a trap: resolved binds those queries to the dummy, whose address black-holes the replies, so it works only intermittently and stalls ~10s per feature downgrade. Don't "fix" a broken outbound integration by pointing the whole box at a public resolver — that breaks every internal name.
